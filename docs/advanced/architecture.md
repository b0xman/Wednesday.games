...
2. This file has a [[<https://en.wikipedia.org/wiki/Shebang_(Unix|shebang]]>) line at the top which tells npm to execute it using Node.
3. `bootstrap-cli.mjs` is responsible for a few things:
   ...
   4. If the `--serve` flag is set, it also starts a file watcher to detect source-code changes (e.g. anything that is `.ts`, `.tsx`, `.scss`, or packager files). On a change, we rebuild the module (step 2 above) using esbuild's [[https://esbuild.github.io/api/#rebuild]] which drastically reduces the build times.
